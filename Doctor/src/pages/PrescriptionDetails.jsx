...
const PrescriptionDetails = () => {
  ...
  const prescriptionRecordId = prescriptionDetails?._id || prescriptionid;
  ...
  const handleUpdateSubmit = async (data) => {
    ...
    try {
      await dispatch(
      updatePrescription({
        prescriptionId: prescriptionRecordId,
        payload: data
      })
      ).unwrap();
      setEditOpen(false);
      dispatch(getPrescriptionDetails(prescriptionRecordId));
    } finally {
      setSaving(false);
    }
  };
  ...
  // Handle Delete
  const handleDelete = () => {
    dispatch(deletePrescription(prescriptionRecordId))
      .unwrap()
      .then(() => {
        setDeleteOpen(false);
  ...
  const { doctordetails, diagonosis, medicines = [], createdAt } =
    prescriptionDetails;
  const diagnoses = String(diagonosis || "")
    .split(/\n|;|,/)
    .map((item) => item.trim())
    .filter(Boolean);
          ...
          <CardContent className="space-y-10 py-8">
            ...
            <section>
              ...
              <div className="mt-3 grid gap-3 sm:grid-cols-2">
                {(diagnoses.length ? diagnoses : ["No diagnosis added"]).map((diagnosis, index) => (
                  <div key={`${diagnosis}-${index}`} className="rounded-xl border border-emerald-100 bg-emerald-50 px-4 py-3">
                    <p className="text-xs font-bold uppercase tracking-[0.12em] text-emerald-600">Diagnosis {index + 1}</p>
                    <p className="mt-1 font-semibold text-slate-900">{diagnosis}</p>
                  </div>
                ))}
              </div>
            </section>
