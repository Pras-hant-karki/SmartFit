...
const adminSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(adminCreateDoctor.fulfilled, (state, action) => {
      state.doctors.push(action.payload);
    });

    builder.addCase(adminDeleteDoctor.fulfilled, (state, action) => {
      state.doctors = state.doctors.filter((doctor) => doctor._id !== action.payload._id);
      if (state.doctorDetails?._id === action.payload._id) {
        state.doctorDetails = null;
      }
    });

    builder.addCase(adminUpdateDoctor.fulfilled, (state, action) => {
