...
const doctorOwnsPrescription = async (prescription, doctor) => {
    if (!doctor) return false;

    const snapshot = prescription.doctordetails || {};
    if (snapshot.doctorusername && snapshot.doctorusername === doctor.doctorusername) return true;
    if (snapshot._id && snapshot._id.toString() === doctor._id.toString()) return true;
...
export const createprescription = asyncHandler(async (req, res) => {
    ...
    const appointment = await Appointment.findById(appointmentid)
        ...
        .populate("doctor", "doctorname doctorusername department specialization qualification email");
    if (!appointment) {
        throw new apiError(404, "Appointment not found");
    }

    if (appointment.doctor._id.toString() !== req.doctor._id.toString()) {
    ...
    const prescription = await Prescription.create({
        ...
        doctordetails: {
            ...
            doctorusername: appointment.doctor.doctorusername,
        ...
        patientdetails: {
            ...
            patientusername: appointment.patient.patientusername,
...
export const getprescription = asyncHandler(async (req, res) => {
    ...
    if (req.doctor && !(await doctorOwnsPrescription(prescription, req.doctor))) {
        throw new apiError(403, "You don't have access to this prescription");
    }
...
export const getallprescriptionsfordoctor = asyncHandler(async (req, res) => {
    ...
    const prescriptions = await Prescription.find({
        $or: [
            ...
            { "doctordetails._id": req.doctor._id },
        ],
    })
        .populate("appointmentid")
        .populate("labtest")
